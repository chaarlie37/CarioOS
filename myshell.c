	...
	while (fgets(buf, 1024, stdin)) {
		...
		for( a = 0; a<contadorProcesosBackground; a++){
			...
			if(procesosBackground[a]->status == 0){
						...
						if(WIFEXITED(status) != 0){
							...
							if(WEXITSTATUS(status) != 0){	// para mandatos que salgan con error o no existan
								...
								if(b == procesosBackground[a]->n_mandatos - 1){
									// si es el ultimo en la mayoria de casos significa que han terminado los demás
									procesosBackground[a]->status = 1;
								}
							}
				...
				// para el caso en el que el ultimo ha terminado, si alguno del medio no ha terminado poner el status a 0
				if(contadorProcesosBackground > 0){
					for( c = 0; c<procesosBackground[a]->n_mandatos; c++){
						if(waitpid(procesosBackground[a]->pids[c], NULL, WNOHANG) == 0){
							procesosBackground[a]->status = 0;
						}
					}
				}
		...
		if(line->ncommands == 1 && strcmp(line->commands[0].argv[0],"fg")==0){
			...
			if(contadorProcesosBackground > 0){
				if(!(line->commands[0].argv[1] == NULL)){
					procesoBg = atoi(line->commands[0].argv[1]);
					if(procesoBg > 0 && procesoBg <= contadorProcesosBackground && procesosBackground[procesoBg-1]->status == 0){
					...
					printf("%s\n", procesosBackground[contadorProcesosBackground-1]->nombre);
